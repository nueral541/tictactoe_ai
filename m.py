...
def minimax(gamestate, depth, is_maxing):
    ...
    if is_maxing:
        ...
        for move in legal_moves(gamestate):
            new_gamestate = copy.deepcopy(gamestate)  # Create a copy of the game state
            new_gamestate[move] = 1
            score = minimax(new_gamestate, depth - 1, False)
        ...
        for move in legal_moves(gamestate):
            new_gamestate = copy.deepcopy(gamestate)  # Create a copy of the game state
            new_gamestate[move] = -1
            score = minimax(new_gamestate, depth - 1, True)
...
def find_best_move(gamestate):
    ...
    for i in range(9):
        if gamestate[i] == 0:
            ...
            score = minimax(gamestate, depth=5, is_maxing=False)  # Assuming opponent's turn
